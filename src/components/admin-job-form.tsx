...
import { Sparkles, Wand2, AlertTriangle, ExternalLink, Pencil } from "lucide-react";
import { useState, useEffect } from "react";
import { extractJobFromUrl, regenerateSummary } from "@/lib/ai.functions";
import { upsertJob } from "@/lib/admin-jobs.functions";
import { checkDuplicateJob } from "@/lib/duplicate-job.functions";
...
export const emptyJobForm: JobFormType = {
...
export function JobForm({ initialData, mode = "new" }: { initialData?: Partial<JobFormType>, mode?: "new" | "edit" }) {
  ...
  const checkDuplicate = useServerFn(checkDuplicateJob);

  const [url, setUrl] = useState("");
  const [form, setForm] = useState<JobFormType>({ ...emptyJobForm, ...initialData });
  ...
  const extractMut = useMutation({
    ...
    onSuccess: async (data: any) => {
      // Phase 2 check (after extraction to check company/title and slug)
      setIsChecking(true);
      try {
        const res = await checkDuplicate({ 
          data: { 
            url: data.apply_url || url, 
            company: data.company, 
            title: data.title,
            exclude_id: form.id
          } 
        });
        console.log(`[Duplicate Check]\nIncoming URL: ${url}\nMatched by: ${res.reason}\nExisting Job ID: ${res.existingJob?.id}\nDecision: ${res.duplicate ? "Duplicate" : "Pass"}`);
        
        if (res.duplicate && mode === "new") {
  ...
  const handleExtractClick = async () => {
    ...
    if (mode === "new") {
      setIsChecking(true);
      try {
        const res = await checkDuplicate({ data: { url } });
  ...
  const set = <K extends keyof JobFormType>(k: K) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) =>
    setForm((f) => ({ ...f, [k]: e.target.value }));

  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold">{mode === "new" ? "Create new job" : "Edit job"}</h1>
        <p className="text-sm text-muted-foreground">
          {mode === "new" 
            ? "Paste the official URL, let AI extract details, review and publish."
            : "Update job details and save."}
        </p>
      </header>

      {/* Step 1: URL extract (always allow extraction, even in edit mode to refresh details) */}
      <section className="glass rounded-2xl p-5">
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row">
          ...
          <button
            ...
            disabled={!url || extractMut.isPending || isChecking}
          ...
          >
            ...
            {isChecking ? "Checking..." : extractMut.isPending ? "Extracting…" : mode === "edit" ? "Re-extract Details" : "Generate Details with AI"}
          </button>
        </div>
        ...
        {duplicateWarning && (
          <div className="mt-4 rounded-xl border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive-foreground">
            <div className="flex items-start gap-3">
              <AlertTriangle className="mt-0.5 h-5 w-5 text-destructive" />
              <div className="flex-1">
                <h3 className="font-semibold text-destructive">Job already exists</h3>
                <div className="mt-2 grid gap-1 text-muted-foreground sm:grid-cols-2">
                  <p><span className="font-medium text-foreground">Title:</span> {duplicateWarning.existingJob.title}</p>
                  <p><span className="font-medium text-foreground">Company:</span> {duplicateWarning.existingJob.company}</p>
                  <p><span className="font-medium text-foreground">Published:</span> {new Date(duplicateWarning.existingJob.created_at).toLocaleDateString()}</p>
                  <p><span className="font-medium text-foreground">Match Reason:</span> {duplicateWarning.reason}</p>
                </div>
                
                <div className="mt-4 flex flex-wrap gap-2">
                  <a
                    href={`/jobs/${duplicateWarning.existingJob.slug}`}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-md bg-background px-3 py-1.5 text-xs font-medium text-foreground shadow-sm hover:bg-accent"
                  >
                  ...
                  <Link
                    ...
                    params={{ slug: duplicateWarning.existingJob.slug }}
                    className="inline-flex items-center gap-1.5 rounded-md bg-background px-3 py-1.5 text-xs font-medium text-foreground shadow-sm hover:bg-accent"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-1.5 text-xs font-medium text-destructive hover:bg-destructive hover:text-destructive-foreground transition-colors"
                  >
                    Continue Anyway
                  </button>
                </div>
      ...
      <section className="glass space-y-4 rounded-2xl p-5">
        ...
        <div className="flex items-center justify-end border-t border-border pt-4">
          ...
          >
            {saveMut.isPending ? "Saving…" : mode === "edit" ? "Save Changes" : "Publish Job"}
          </button>
        </div>
      </section>
    </div>
