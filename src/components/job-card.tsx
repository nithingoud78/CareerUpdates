...
export function JobCard({ job, compact = false }: { job: Job; compact?: boolean }) {
  return (
    <Link
      ...
      className="group glass relative flex flex-col gap-3 rounded-2xl p-4 transition-all hover:border-brand/50 hover:shadow-lg hover:shadow-brand/5"
    >
      ...
      <div className="mt-auto flex items-center justify-between gap-2 pt-1">
        ...
        <span className="rounded-full bg-brand px-3 py-1.5 text-xs font-semibold text-brand-foreground transition-transform group-hover:scale-105">
          Apply Now
        </span>
      </div>
    </Link>
