...
export const Route = createFileRoute("/_authenticated/admin/resume-templates/$id/")({
  component: AdminTemplateOverview,
});

function AdminTemplateOverview() {
  ...
  const template = response.product as any;
  const isPublished = template.status === "published";
  const isArchived = template.status === "archived";
  const publicUrl = `${window.location.origin}/resume-templates/${template.slug}`;
  ...
  const adminActions = (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <button onClick={() => { if(confirm("Delete this template?")) remove.mutate(); }} className="flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-5 py-2 text-sm font-medium text-red-600 hover:bg-red-100 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-400">
