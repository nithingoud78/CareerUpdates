...
export const Route = createFileRoute("/checkout/bundle/$slug")({
  ...
  loader: async ({ params }) => {
    ...
    return { product: res.product, resources: res.resources };
  },
});
...
function BundleCheckoutPage() {
  const { product, resources } = Route.useLoaderData();
      ...
      <div className="grid gap-12 md:grid-cols-2">
        ...
        <section className="h-fit rounded-2xl border border-border bg-muted/20 p-6 md:p-8">
          ...
          <div className="mt-4 border-b border-border pb-4">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Included Resources</h4>
            <ul className="space-y-1.5">
              {resources.map((r: any) => (
                 <li key={r.id} className="text-sm text-foreground flex items-center gap-2">
          ...
          <div className="mt-6 flex justify-between border-t border-border pt-6">
            ...
            <span className="text-xl font-bold text-brand">{product.is_free ? "₹0" : `₹${product.current_price}`}</span>
          </div>
        </section>
      </div>
    </main>
