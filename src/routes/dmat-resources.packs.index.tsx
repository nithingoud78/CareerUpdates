...
import { getPublishedDmatPacks } from "@/lib/career-tools.functions";
import { ScrollReveal } from "@/components/scroll-reveal";

const SITE_URL = "https://careerupdates.co.in";

export const Route = createFileRoute("/dmat-resources/packs/")({
  head: () => ({
    meta: [
      { title: "dMAT Complete Packs | Career Updates" },
      { name: "description", content: "Get complete dMAT preparation packs." },
      { property: "og:title", content: "dMAT Complete Packs | Career Updates" },
      { property: "og:type", content: "website" },
      { property: "og:url", content: `${SITE_URL}/dmat-resources/packs` },
      { property: "og:image", content: `${SITE_URL}/careerupdates-share-2026.png` },
      { name: "twitter:card", content: "summary_large_image" },
    ],
    links: [{ rel: "canonical", href: `${SITE_URL}/dmat-resources/packs` }],
  }),
  component: DmatPacksList,
});

function DmatPacksList() {
  const getBundles = useServerFn(getPublishedDmatPacks);
  const { data: bundles, isLoading } = useQuery({
    queryKey: ["published-dmat-packs"],
    queryFn: () => getBundles(),
  });
      ...
      <section className="relative overflow-hidden">
        ...
        <ScrollReveal className="mx-auto max-w-4xl px-4 pb-10 pt-12 text-center sm:px-6">
          ...
          <h1 className="text-balance text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
            dMAT <span className="text-brand">Complete Packs</span>
          </h1>
          <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground sm:text-base">
            Get everything you need in one comprehensive bundle. Save money and get all modules bundled together.
          </p>
          <div className="mt-4 flex flex-wrap justify-center gap-3">
            ...
            >
              <FileText className="h-3.5 w-3.5" /> View Individual Modules
            </Link>
          </div>
        </ScrollReveal>
      </section>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        ...
        {isLoading && (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((n) => (
              <div key={n} className="glass animate-pulse rounded-3xl p-5">
                <div className="mb-4 aspect-[4/3] w-full rounded-2xl bg-muted/50" />
        ...
        {!isLoading && bundles && bundles.length > 0 && (
          <ScrollReveal delay={0.1}>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {bundles.map((b) => (
                <BundleCard key={b.id} bundle={b as any} baseUrl="/dmat-resources/packs/$slug" />
              ))}
            </div>
          </ScrollReveal>
        )}
